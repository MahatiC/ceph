...
void C_BlockIORequest<T>::complete_user_request(int r) {
  ...
  if (m_user_req_completed.compare_exchange_strong(initial, true)) {
    ...
    if (ssd_writelog) {
      pwl.queue_ctx(user_req, r);
      //pwl.m_image_ctx.op_work_queue->queue(user_req, r);
    } else {
      user_req->complete(r);
...
C_WriteRequest<T>::C_WriteRequest(T &pwl, const utime_t arrived, io::Extents &&image_extents,
  ...
  : C_BlockIORequest<T>(pwl, arrived, std::move(image_extents), std::move(bl), fadvise_flags, user_req, ssd_writelog),
...
void C_WriteRequest<T>::setup_buffer_resources(
  ...
  if (this->ssd_writelog) {
    pwl.update_resources(this, bytes_cached, bytes_dirtied, bytes_allocated,
...
C_DiscardRequest<T>::C_DiscardRequest(T &pwl, const utime_t arrived, io::Extents &&image_extents,
                                      ...
                                      PerfCounters *perfcounter, Context *user_req, bool ssd_writelog)
  : C_BlockIORequest<T>(pwl, arrived, std::move(image_extents), bufferlist(), 0, user_req, ssd_writelog),
...
C_CompAndWriteRequest<T>::C_CompAndWriteRequest(T &pwl, const utime_t arrived, io::Extents &&image_extents,
                                                ...
                                                Context *user_req, bool ssd_writelog)
  : C_WriteRequest<T>(pwl, arrived, std::move(image_extents), std::move(bl), fadvise_flags, lock, perfcounter, user_req, ssd_writelog),
...
template class librbd::cache::pwl::C_BlockIORequest<librbd::cache::AbstractWriteLog<librbd::ImageCtx> >;
template class librbd::cache::pwl::C_WriteRequest<librbd::cache::AbstractWriteLog<librbd::ImageCtx> >;
template class librbd::cache::pwl::C_FlushRequest<librbd::cache::AbstractWriteLog<librbd::ImageCtx> >;
template class librbd::cache::pwl::C_DiscardRequest<librbd::cache::AbstractWriteLog<librbd::ImageCtx> >;
template class librbd::cache::pwl::C_WriteSameRequest<librbd::cache::AbstractWriteLog<librbd::ImageCtx> >;
template class librbd::cache::pwl::C_CompAndWriteRequest<librbd::cache::AbstractWriteLog<librbd::ImageCtx> >;
